...
function showMessages() {
    ...
    document.getElementById("password").style.displa
}
...
login_button.addEventListener('click', function() {
    ...
    status_text.textContent = response;
...
window.addEventListener("load", function () {
    ...
    xhr.onload = function() {
        if (xhr.status === 200) {
            status_text.textContent = xhr.responseText;
            // maybe show the message UI, hide login/register fields
        } else {
            status_text.textContent = "Please log in.";
            // maybe show login/register UI
        }
    };
    xhr.send();
});
